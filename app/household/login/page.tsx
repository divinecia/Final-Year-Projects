...
export default function HouseholdLoginPage() {
  ...
  const handleSocialSuccess = async (userId: string, _email: string) => {
    ...
    try {
      // Check if user exists in households collection
      const { doc, getDoc } = await import("firebase/firestore");
      const userDoc = await getDoc(doc(db, "households", userId));
        ...
        <div className="w-full max-w-md space-y-4">
            <Card className="w-full">
                <CardHeader className="text-center">
                    <CardTitle>Login to Your Household Account</CardTitle>
                    <CardDescription>Access your dashboard and manage your bookings.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Form {...form}>
            ...
            <div className="mt-4 text-center text-sm text-muted-foreground">
                ...
                <Link href="/household/register" className="font-semibold text-primary hover:underline">
                    Sign up here
                </Link>
            </div>
        </div>
